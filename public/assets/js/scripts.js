...
(function($) {
	...
	jQuery(document).on('ready', function(){
		...
		$('#send_message_btn').on('click', function(e) {
			...
			$("#name_error").text(!name?"Name is required":"")
			$("#phone_error").text(!phone||phone.trim().length<10?"Phone Number is not correct":"")
			$("#subject_error").text(!subject?"Subject is required":"")
			$("#message_error").text(!msg?"Message is required":"")

			if(!($("#name_error").text()||$("#phone_error").text()||$("#subject_error").text()||$("#message_error").text())){
				//send message
				$.ajax({
					method: 'POST',
					url: 'https://formsubmit.co/ajax/2aa7f68806eb8393c087f8d10fb734f8',
					dataType: 'json',
